...
class BCDataset(IterableDataset):
    def __init__(
        ...
        for _path_idx in self._paths:
            ...
            traj_actions = data["actions"]  # dataset actions
            ...
            for i in range(min(num_demos_per_task, len(observations))):
                ...
                # TODO: consider images, depth, points for different settings
    ...
    def _sample(self):
        ...
        traj_len = len(observations[f"{self._robot_points_key}_3d"])

        # Sample obs, action
        sample_idx = np.random.randint(0, traj_len)

        if self._obs_type in ["image", "depth"]:
        ...
        if "points" in self._obs_type:
            ...
            # Apply noise to object points if enabled
            if self._noise_object_points:
                past_object_points = past_object_points + np.random.normal(
                    0, self._noise_std, past_object_points.shape
                )
        ...
        if self._action_mode == "points":
            ...
            # repeat last dimension of ft to self._mult_fact_robot_points
            ...
            if self._action_chunking:
                ...
                # start_idx = min(sample_idx + self._act_subsample, len(actions) - 1)
