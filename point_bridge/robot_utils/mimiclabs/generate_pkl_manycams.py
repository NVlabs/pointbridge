...
This script generates multiple observations for each demo, one for each camera pose
in multi_cam_poses. If there are N demos and M camera poses, the output will contain
N*M observations (N observations for each of the M camera poses).
"""
...
from point_bridge.robot_utils.mimiclabs.utils import camera2pixelkey, pixelkey2camera, T_robot_base, T_gripper, sample_cuboid_shell
...
save_pixels = False
save_dir_suffix = "manycams"
add_multi_cam = True
add_depth_from_multi_cam = True
...
calib_dir = HOME_DIR / "point_bridge/robot_utils/mimiclabs/calibration_matrices"

if add_depth_from_multi_cam:
...
if add_multi_cam:
    multi_cam_poses = sample_cuboid_shell(
        inner_min=np.array([-0.01, -0.2, 0.0]),
        inner_max=np.array([0.8, 0.2, 0.6]),
        outer_min=np.array([-0.05, -0.5, 0.6]),
        outer_max=np.array([1.0, 0.5, 0.8]),
        nx=100,
        ny=100,
        nz=100,
        target=np.array([0.6, 0.0, 0.0]),
        fps_func=farthest_point_sampling,
        reduce_to_num_poses=20,
    )  # all coordinates are in meters in robot base frame
    num_cam_pose_per_trajectory = 5

    camera_names += ["cam_multi"]
    pixel_keys += ["pixels_cam_multi"]
    pixelkey2camera["pixels_cam_multi"] = "cam_multi"
    camera2pixelkey["cam_multi"] = "pixels_cam_multi"
...
for task_idx, task_name in enumerate(task_names):
    ...
    for demo_idx, demo_key in enumerate(data.keys()):
        ...
        try:
            # Loop over all camera poses instead of randomly selecting one
            if add_multi_cam:
                num_camera_poses = num_cam_pose_per_trajectory
                sampled_cam_pose_indices = np.random.choice(
                    range(len(multi_cam_poses)),
                    size=num_cam_pose_per_trajectory,
                    replace=False,
                )
            for enumerate_idx, cam_pose_idx in enumerate(sampled_cam_pose_indices):
                print(
                    f"Processing demo {demo_idx + 1}/{len(data.keys())}, camera pose {enumerate_idx + 1}/{num_camera_poses}: {demo_key}"
                )
                ...
                for step_idx in range(len(demo_data["states"])):
                    ...
                    if step_idx == 0:
                        ...
                        # add from extrinsics
                        if add_multi_cam:
                            # Use the current camera pose from the loop instead of random selection
                            camera_pose = multi_cam_poses[cam_pose_idx]

                            # save to json under camera name cam_multi
                            camera_extrinsics_json = {
                                "cam_multi": {
                                    "rotation": camera_pose[:3, :3].tolist(),
                                    "translation": camera_pose[:3, 3].tolist(),
                                }
                            }
                            directory = Path(calib_dir)
                            directory.mkdir(parents=True, exist_ok=True)
                            with open(directory / "camera_extrinsics.json", "w") as f:
                                json.dump(camera_extrinsics_json, f, indent=4)
                            ...
                            (
                                ...
                                env,
                                directory / "camera_extrinsics.json",
                                T_robot_base,
                                camera_names=["cam_multi"],
                            )
                            if "pixels_cam_multi" not in _pixel_keys:
                                pixel_keys.append("pixels_cam_multi")
                                camera_names.append("cam_multi")
                                pixelkey2camera["pixels_cam_multi"] = "cam_multi"
                                camera2pixelkey["cam_multi"] = "pixels_cam_multi"
                    else:
                    ...
                    for save_idx, saved_points_per_obj in enumerate(
                        SAVED_POINTS_PER_OBJ
                    ):
                        save_key_name = f"object_tracks_{saved_points_per_obj}"
                        ...
                        for camera_name in camera_names:
                            ...
                            if add_multi_cam and add_depth_from_multi_cam:
                                ...
                                for obj_idx, obj_pts in enumerate(points3d_objects):
                                    ...
                                    for i, point_2d in enumerate(
                                        points2d_objects[obj_idx]
                                    ):
                    ...
                    T = (
                        np.linalg.inv(robot_base) @ T
                    )  # FR3 gripper in FR3 robot base frame
                    eef_pos, eef_ori = T[:3, 3], T[:3, :3]
                    eef_quat = R.from_matrix(eef_ori).as_quat()
                    observation["eef_states"].append(
                        np.concatenate([eef_pos, eef_quat])
                    )
                    observation["gripper_states"].append(
                        demo_data["actions"][step_idx, -1]
                    )
                ...
                observation["robot_tracks_3d"] = np.array(
                    observation["robot_tracks_3d"]
                )
                for saved_points_per_obj in SAVED_POINTS_PER_OBJ:
                    save_key_name = f"object_tracks_{saved_points_per_obj}"
                ...
                if reward == 1.0:
                    ...
                    print(
                        f"Skipping demo {demo_key} with camera pose {cam_pose_idx} because final reward is {reward}"
                    )
