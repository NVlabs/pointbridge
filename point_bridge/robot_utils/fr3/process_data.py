...
parser.add_argument(
    ...
    type=str,
    default="en",
    help="Language for the video (provide language intruction with underscores)",
)
...
for TASK_NAME in task_names:
    ...
    # save label.txt file
    with open(f"{SAVE_PATH}/{TASK_NAME}/label.txt", "w") as f:
        lang_instruction = args.lang
        lang_instruction = " ".join(lang_instruction.split("_"))
        f.write(f"{lang_instruction}")
    ...
    for num, demo_dir in enumerate(sorted(DEMO_DIRS)):
        ...
        def save_only_videos(base_folder_path, depth=False):
            ...
            for cam in cam_indices:
                if depth and (cam == 51 or cam in zed_indices):
                    continue
