...
class DiffusionPolicy(nn.Module):
    ...
    def __init__(
        ...
        if self.policy_type == "cnn":
            if pred_horizon == 10:  # TODO: is it correct???
                self.pad_before = 3
                self.pad_after = 3
        ...
        self.num_inference_iters = num_inference_iters or num_diffusion_iters
    ...
    def forward(
        self, obs_seq: torch.Tensor, action_seq: Optional[torch.Tensor]
            ...
            obs_seq = torch.cat(
                (
                    torch.tile(
                        ...
                        (
                            1,
                            self.obs_horizon - obs_seq.shape[1],
                            1,
                        )  # NOTE: Changed to below
                        # obs_seq[:, 0, :],
                        (1, self.obs_horizon - obs_seq.shape[1], 1),
                    ),
                    obs_seq,
                ),
                dim=-2,
    ...
    def _predict(
        ...
        if obs_seq.shape[1] < self.obs_horizon:
            obs_seq = torch.cat(
                (
                    torch.tile(
                        obs_seq[:, :1, :], (1, self.obs_horizon - obs_seq.shape[1], 1)
                    ),
                    obs_seq,
                ),
                dim=-2,
        ...
        with torch.no_grad():
            ...
            # init scheduler
            self.noise_scheduler.set_timesteps(self.num_inference_iters)
